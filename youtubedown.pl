...
my ($version) = ('$Revision: 1.882 $' =~ m/\s(\d[.\d]+)\s/s);
...
sub load_twitter_formats($$) {
  ...
  errorI ("$id: video ID not found") unless ($id2);
  my $src = "https://pbs.twimg.com/tweet_video/$id2.mp4";
...
sub canonical_url($;) {
  ...
  # Tumblr /post/NNNNN
  } elsif ($url =~ m@^https?://([-_a-z\d]+)\.(tumblr)\.com/.*?/(\d{8,})/@si) {
    my $user;
    ($user, $site, $id) = ($1, $2, $3);
    $site = lc($site);
    $url = "https://$user.$site.com/post/$id";
